...
export default function List() {
    ...
    const getListData = async () => {
        let list = await fetch('https://todo-app-mern-back1.onrender.com/tasks')
    ...
    let item = await fetch(
        'https://todo-app-mern-back1.onrender.com/delete/' + id,
        {
            method: 'DELETE'
        }
    )
    ...
    const selectedSingleItem = (id) => {
        console.log(id);
        if (selectedTask.includes(id)) {
            let items = selectedTask.filter(
                (item) => item != id);
            setSelectedTask(items)
        } else {
    ...
    const deleteMultiple = async () => {
     ...
     let item = await fetch(
"https://todo-app-mern-back1.onrender.com/delete-multiple/",
{
    method:"DELETE",
...

            {
                taskData && taskData.map((item, index) => (
                    <Fragment key={item._id}>
                        <li className="list-item"><input onChange={() => selectedSingleItem(item._id)} checked={selectedTask.includes(item._id)} type="checkbox" /></li>
